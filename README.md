...
This RAT enables arbitrary shell execution, uses a robust multi-process watchdog architecture and operates through a command-and-control (C2) infrastructure based on AMQP/RabbitMQ. The payload, identified as `bc20`, primarily monetizes through ad injection while providing the operator with continuous remote shell access.
